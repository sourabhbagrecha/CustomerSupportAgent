...
**Audit view** (`web/src/components/AuditPanel.tsx`, `GET /api/approvals/pending`, `GET /api/ledger`): the trace panel is per conversation, so the UI has a second tab that reads across all of them. It lists every approval currently waiting for a human, on any thread, and lets an operator approve or reject in place through the same route the chat banner uses; below it sits the full `actions_ledger`, filterable by status, so the "a ledger row is written before any money moves" invariant is visible rather than only assertable. Denied and `awaiting_approval` rows appear there too, which is the point: the rows where no money moved are the ones that prove the policy engine ran.
...
- The audit tab is a read-only observability surface plus the pre-existing approve/reject action, not admin CRUD: nothing there creates, edits, or deletes a row, and there is no operator identity beyond the existing hardcoded `resolved_by = "human_agent"`, consistent with the single-tenant no-auth assumption above. Its pending-approvals queue refreshes on mount, on tab focus, and on a 10-second poll, and it displays its own last-refreshed time rather than streaming, because the SSE channel is per conversation by design and a cross-thread queue has no conversation to subscribe to.
...
Plus 50 deterministic unit tests (`npm run test`, no LLM calls) covering the policy engine, idempotency key derivation, ledger/reconciliation transitions, ledger and approval-queue queries, HTTP query-parameter validation, retrieval ranking, and `policy.md`/`policy.json` consistency.
...
- The approval resolve route commits the human's decision (`resolveApproval`) before it resumes the graph, deliberately, so the decision is recorded independently of whatever the downstream money outcome turns out to be. If the process dies between those two steps, the approvals row is resolved while its ledger row is still `awaiting_approval` and the LangGraph thread is still interrupted, with no recovery path: the conversation cannot be continued and the money never moved. The audit tab is the first surface that makes such an orphan visible (an `awaiting_approval` ledger row with nothing waiting in the queue). We left the ordering alone rather than reversing it, because the opposite ordering trades this for a worse failure: a completed payment whose approval record was never written.

...
4. Point out the current gate result: 21 of 22 scenarios passing, with scenario 5 (`vague-compensation-clarification`) as the one documented open failure (see docs/failure-modes.md for why).
...
- Every fault toggle and the Reset demo button are demo-only conveniences, unauthenticated by design (single-tenant, no-auth scope; see docs/assumptions.md); do not read them as production affordances.
